...
public class GlobalApiExceptionHandler {
    ...
    public ResponseEntity<Map<String, String>> handleDuplicateKey(org.springframework.dao.DuplicateKeyException ex) {
        // Unique-constraint collisions (e.g. re-running idempotent workflows) are a
        // conflict, not a server bug.
        String rootMessage = ex.getMostSpecificCause() == null ? ex.getMessage() : ex.getMostSpecificCause().getMessage();
        return ResponseEntity.status(HttpStatus.CONFLICT).body(Map.of("error", rootMessage == null ? "Duplicate key" : rootMessage));
    }
}
