...
public class RetentionService {
    ...
    public RetentionService(JdbcTemplate jdbcTemplate, RetentionPolicy retentionPolicy) {
    ...
    public int purgeExpiredAuditLogs() {
        return jdbcTemplate.update(
            ...
            where al.created_at < date_sub(current_timestamp, interval ? year)
              and rh.id is null
            """,
            retentionPolicy.auditRetentionYears()
        );
    }
    ...
    public int purgeExpiredAuditRedactionEvents() {
        return jdbcTemplate.update(
            ...
            where ared.created_at < date_sub(current_timestamp, interval ? year)
              and rh.id is null
            """,
            retentionPolicy.auditRetentionYears()
        );
    ...
    public int purgeExpiredReconciliationExceptions() {
        return jdbcTemplate.update(
            ...
            where re.created_at < date_sub(current_timestamp, interval ? year)
              and rh.id is null
            """,
            retentionPolicy.reconciliationRetentionYears()
        );
    }
    ...
    public int purgeExpiredReconciliationRuns() {
        return jdbcTemplate.update(
            ...
            where coalesce(rr.completed_at, rr.started_at) < date_sub(current_timestamp, interval ? year)
              and rh.id is null
              and not exists (
                  select 1
                  from reconciliation_exception re
